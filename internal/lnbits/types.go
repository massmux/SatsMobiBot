...
type User struct {
	...
	BreezMnemonic    string       `json:"breez_mnemonic"`    // User's Breez wallet mnemonic (encrypted with PIN)
...
// IsPinLocked returns true if user is temporarily locked out due to failed attempts
func (u *User) IsPinLocked() bool {
	return u.PinLockedUntil != nil && time.Now().Before(*u.PinLockedUntil)
}
